...
var jwtKeyStr = builder.Configuration["Jwt:Key"]
    ?? throw new InvalidOperationException(
        "Jwt:Key não configurado. Dev: rode 'dotnet user-secrets set Jwt:Key <valor-aleatorio>' na pasta GECManager.API. " +
        "Produção: defina a variável de ambiente Jwt__Key. Nunca commitar a chave no appsettings.");
...
// Migrações versionadas (dbs antigos criados com EnsureCreated devem ser apagados em dev)
...
// Em dev o Web (Blazor) chama http://localhost:7000; o redirect para HTTPS
// quebra o preflight CORS (OPTIONS não segue redirect)
if (!app.Environment.IsDevelopment())
